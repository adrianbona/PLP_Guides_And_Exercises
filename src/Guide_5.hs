...
------------------------- ABS    ------------------------------------ ?
-- ø ⊢ λx.λx.x ↪ ⟨x, λx.x, ø⟩    x = ⟨2, ø⟩ ⊢ λx.x ↪ ⟨x, x, x = ⟨2, ø⟩⟩
-------------------------------------------------------------------- MN    --------------------------------- ?
-- ø ⊢ (λx.λx.x) 2 ↪ ⟨x, x, ø⟩                                             x = ⟨3, ø⟩ ⊢ x ↪ ⟨x, x, x = ⟨3, ø⟩⟩
----------------------------------------------------------------------------------------------------------- MN
-- ø ⊢ (λx.λx.x) 2 3 ↪ ⟨x, x, x = ⟨3, ø⟩⟩
...
--------------------------------------------- ABS    --------------------------------------------- ?
-- ø ⊢ λx.(λf.(λy.f 6) 5) ↪ ⟨x, λf.(λy.f 6) 5, ø⟩    x = ⟨λy.if isZero(y) then x else y, ø⟩ ⊢ M' ↪ V
----------------------------------------------------------------------------------------------------MN     ----------------- ?
-- ø ⊢ λx.(λf.(λy.f 6) 5) (λy.if isZero(y) then x else y)                                                  x = ⟨4, ø⟩ ⊢ M' ↪ V
--------------------------------------------------------------------------------------------------------------------------- MN
